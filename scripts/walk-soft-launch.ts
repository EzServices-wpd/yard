/**
 * Soft-launch honesty walk — geometry + plan quality for all 10 launch builds.
 * Photo/enrich need a browser (WebGL); this script gates size, steps, BOM, cut list, nest eligibility.
 * Run: npx tsx --tsconfig tsconfig.json scripts/walk-soft-launch.ts
 */
...
const BUILDS: {
  ...
  prompt: string;
  house: boolean;
  expect?: { w?: number; h?: number; d?: number; kind?: string; nameIncludes?: string };
}[] = [
  {
    id: "1-pocket",
    house: true,
    prompt: `I have a pocket space in my bathroom with these exact dimensions:
...
Centered vanity with open knee space (≈22 inches clear) under a counter at 34 inches high. Drawers on either side of the knee space. Upper cabinetry from 54 inches to the ceiling (102"). Large doors with adjustable shelving for towels and linens. Mirror and storage beside the chair space. Structurally centered and anchored into studs.`,
    expect: { kind: "closet", nameIncludes: "vanity" },
  },
  {
    id: "2-linen",
    house: true,
  ...
  },
  {
    ...
    expect: { w: 60, h: 29, d: 30, kind: "closet", nameIncludes: "Desk" },
  },
  {
    id: "5-pantry",
    house: true,
    prompt: "hall pantry 24 wide by 84 tall by 14 deep, 5 shelves, 3/4 inch plywood",
    expect: { w: 24, h: 84, d: 14, nameIncludes: "Pantry" },
  },
  {
    id: "6-bench",
    house: true,
    prompt: "mudroom bench 48 wide by 18 deep by 18 high with 3 cubbies",
    expect: { w: 48, h: 18, d: 18, nameIncludes: "Bench" },
  },
  {
    id: "7-bookcase",
    house: true,
    prompt: "kids bookcase 30 wide by 11 deep by 48 high, 4 shelves",
    expect: { w: 30, h: 48, d: 11, nameIncludes: "Bookcase" },
  },
  {
    id: "8-media",
    house: true,
    prompt: "media console 60 wide by 16 deep by 24 high, two doors, open center",
    expect: { w: 60, h: 24, d: 16, nameIncludes: "Media" },
  },
  {
    id: "9-eiffel",
    house: false,
...
const TERMS =
  /carcase|toekick|dry-fit|overlay|lag|edge banding|shim|scribe|predrill|plumb|rack/i;
...
for (const b of BUILDS) {
  ...
  const generic = plan.instructions.filter((s) =>
    /assemble the carcase|square and label|cut the sheet/i.test(s.title),
  );
  if (generic.length) errs.push(`generic titles: ${generic.map((s) => s.title).join(", ")}`);
  const cutNums = plan.instructions.some(
    (s) => /\d+(?:\.\d+)?"/.test(s.description) || /\d+×/.test(s.description) || /\d+ ×/.test(s.description),
  );
  ...
  if (b.house && project.panels.length > 0) {
    const joined = plan.instructions.map((s) => `${s.title} ${s.description} ${s.tips ?? ""}`).join(" ");
    if (!TERMS.test(joined)) errs.push("no shop terms in steps (carcase/toekick/dry-fit/…)");
    if (plan.partsKind !== "whole") {
      const nest = nestCutList(plan.cutList);
      if (!nest || !nest.sheets.length) errs.push("house ply cut list produced no nest sheets");
    }
  }
  ...
  const line = `${flag} ${b.id.padEnd(12)} ${project.name.padEnd(28)} ${pieces.toString().padStart(4)} pcs  ${plan.instructions.length} steps  ~$${Math.round(plan.totals.estCostUsd)}`;
  console.log(line);
  rows.push(line);
  for (const e of errs) {
    console.log(`     - ${e}`);
  ...
  if (!errs.length) {
    const titles = plan.instructions.map((s) => s.title).join(" → ");
    console.log(`     steps: ${titles}`);
    rows.push(`     steps: ${titles}`);
  }
}

console.log(failed ? `\n${failed} failed` : "\n10/10 soft-launch geometry green");
