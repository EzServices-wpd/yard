...
export function checkedUrl(url) {
  ...
  if (!LOOPBACK_HOSTNAMES.has(parsed.hostname) && process.env.BROWSER_ALLOW_EXTERNAL_HOST !== "1") {
    fail(`${parsed.hostname} is not a loopback host; these scripts screenshot the local dev server. Set BROWSER_ALLOW_EXTERNAL_HOST=1 to override.`);
  }
  return url;
}
...
export function checkedOutputPath(outPng, allowedDirs) {
  ...
  if (!allowed) fail(`screenshot path must be under ${allowedDirs.join(" or ")}, got ${abs}`);
  return abs;
}
