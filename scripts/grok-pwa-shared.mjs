...
export function injectGrokPwaHead(html, appName = DEFAULT_APP_NAME, projectId = readGrokProjectId(), creator = readXCreator(), creatorId = readXCreatorId()) {
...
export function createHeadInjector(appName = DEFAULT_APP_NAME, projectId = readGrokProjectId(), creator = readXCreator(), creatorId = readXCreatorId()) {
  let pending = [];
  let done = false;
  return {
    push(chunk) {
