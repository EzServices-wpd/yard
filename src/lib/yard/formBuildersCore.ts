...
export function pyramidOps(s: Size3): FormOp[] {
  ...
  // denser envelope preview so the stepped silhouette reads before graph build
  const n = 12;
  const ops: FormOp[] = [];
  for (let i = 0; i <= n; i++) {
    const u = i / n;
    // match buildSteppedPyramid base bias so ghost and graph agree
    const t = u * 0.45 + (u * u * (3 - 2 * u)) * 0.55;
    const y = t * H;
    const h = half + (tip - half) * (y / H);
    ops.push({
      op: "poly",
