...
export function detectMaterial(prompt: string): CatalogItem {
  ...
  const phrases: [RegExp, string][] = [
    ...
    [/toothpick/, "toothpick-wood"],
    [/drinking straw|plastic straw|straw/, "straw-plastic"],
    ...
    [/dowel/, "dowel-1-4"],
    [/plywood|sheet goods/, "plywood-3-4"],
  ...
  // soft defaults by structure
  const kind = detectStructure(lower);
  const soft: Partial<Record<StructureKind, string>> = {
    eiffel: "popsicle-standard",
    pyramid: "popsicle-standard",
    bridge: "popsicle-standard",
    arch: "pvc-3-4-sch40",
    furniture: "lumber-1x4-8",
    ladder: "lumber-1x4-8",
    closet: "plywood-3-4",
  };
  const id = soft[kind] || "popsicle-standard";
  return getCatalogItem(id) || FORGE_CATALOG[0];
}
