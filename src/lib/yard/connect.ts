...
export function joinTol(item: CatalogItem): number {
  ...
  const base = craft ? d.thick * 3.4 : d.thick * 2.4;
  return Math.max(base, craft ? 0.42 : 0.32);
}
...
export function analyzePieces(instances: YardInstance[], item: CatalogItem): BuildStats {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (len > tol * 4) {
      // Extreme craft spans (long straws / sticks) need denser sampling so
      // mid-span T-joins still register as one component.
      const steps = Math.min(10, Math.max(1, Math.floor(len / Math.max(cell * 1.2, tol * 1.6))));
