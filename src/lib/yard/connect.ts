...
export function joinTol(item: CatalogItem): number {
  ...
  // Craft stock (straws, sticks, toothpicks) needs a slightly looser
  // visual join radius so T-meets and near-miss laps still count as connected.
  // Ultra-thin extreme craft (toothpick / fine straw) gets a higher floor so
  // residual multi-component on long spans collapses without inventing joints.
...
/** True if two segments come within tol (T-join / near cross). */
function segNearSeg(a0: Vec3, a1: Vec3, b0: Vec3, b1: Vec3, tol: number): boolean {
  // denser samples catch near-miss T-joins on long thin craft members
...
export function analyzePieces(instances: YardInstance[], item: CatalogItem): BuildStats {
  ...
  // Sample endpoints + midpoints of long members so mid-span T-joins
  // (common on straw / stick lattices) enter the spatial hash.
  for (let i = 0; i < n; i++) {
    const A = segs[i];
    ...
    if (len > tol * 3.2) {
      // Extreme craft spans (long straws / sticks / toothpicks) need denser
      // sampling so mid-span T-joins still register as one component.
      const stepPitch = Math.max(cell * 0.95, tol * 1.25);
      const steps = Math.min(14, Math.max(1, Math.floor(len / stepPitch)));
      for (let s = 1; s < steps; s++) {
        const t = s / steps;
        const p = lerp(A.a, A.b, t);
        put(p.x, p.y, p.z, i);
      }
    }
  }
  ...
  const consider = (i: number, j: number) => {
    ...
    const hit =
      ...
      // Mid-span cross / T: sample a few points along the longer member
  ...
  for (let i = 0; i < n; i++) {
    const A = segs[i];
    const samples: Vec3[] = [A.a, A.b];
    const len = dist(A.a, A.b);
    if (len > tol * 4) {
      samples.push(mid(A.a, A.b));
      samples.push(lerp(A.a, A.b, 0.25));
      samples.push(lerp(A.a, A.b, 0.75));
    }
