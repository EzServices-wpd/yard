...
const SUBJECTS: { re: RegExp; id: string; label: string }[] = [
  ...
  { re: /\b(dinosaur|dino|t[\s-]?rex|trex)\b/, id: "dinosaur", label: "Dinosaur" },
  { re: /\b(castle|fort|fortress|keep)\b/, id: "castle", label: "Castle" },
  { re: /\b(monogram|initial|letter\s*[a-z]|letter)\b/, id: "monogram", label: "Monogram" },
  { re: /\b(butterfly|moth)\b/, id: "butterfly", label: "Butterfly" },
];
...
export function detectFlatPrompt(prompt: string): FlatIntent | null {
  ...
  const flatCue =
    ...
    (/\bpaper\b/.test(lower) &&
      /\b(car|house|tree|star|rocket|heart|dog|boat|plane|flower|fish|person|kid|child|dinosaur|dino|castle|fort|monogram|butterfly|cat)\b/.test(
        lower,
      ));
...
function silhouette(subject: string): Pt[][] {
  ...
  // NEW silhouettes — full paths in next commit if this lands; subjects already detected
  if (s === "dinosaur" || s.includes("dino") || s.includes("trex")) {
    return [
      [
        { x: 0.22, y: 0.38 }, { x: 0.38, y: 0.52 }, { x: 0.62, y: 0.52 },
        { x: 0.78, y: 0.42 }, { x: 0.72, y: 0.32 }, { x: 0.28, y: 0.32 }, { x: 0.22, y: 0.38 },
      ],
      [
        { x: 0.72, y: 0.42 }, { x: 0.82, y: 0.58 }, { x: 0.9, y: 0.62 },
        { x: 0.94, y: 0.55 }, { x: 0.86, y: 0.5 }, { x: 0.78, y: 0.42 },
      ],
      [{ x: 0.3, y: 0.32 }, { x: 0.28, y: 0.12 }],
      [{ x: 0.4, y: 0.32 }, { x: 0.42, y: 0.12 }],
      [{ x: 0.55, y: 0.32 }, { x: 0.54, y: 0.12 }],
      [{ x: 0.66, y: 0.32 }, { x: 0.68, y: 0.12 }],
      [
        { x: 0.22, y: 0.35 }, { x: 0.1, y: 0.28 }, { x: 0.06, y: 0.22 },
        { x: 0.12, y: 0.18 }, { x: 0.22, y: 0.28 },
      ],
    ];
  }
  if (s === "castle" || s.includes("castle") || s.includes("fort")) {
    return [
      [
        { x: 0.12, y: 0.12 }, { x: 0.12, y: 0.42 }, { x: 0.88, y: 0.42 },
        { x: 0.88, y: 0.12 }, { x: 0.12, y: 0.12 },
      ],
      [
        { x: 0.1, y: 0.12 }, { x: 0.1, y: 0.72 }, { x: 0.28, y: 0.72 },
        { x: 0.28, y: 0.12 }, { x: 0.1, y: 0.12 },
      ],
      [{ x: 0.1, y: 0.72 }, { x: 0.1, y: 0.82 }],
      [{ x: 0.16, y: 0.72 }, { x: 0.16, y: 0.82 }],
      [{ x: 0.22, y: 0.72 }, { x: 0.22, y: 0.82 }],
      [{ x: 0.28, y: 0.72 }, { x: 0.28, y: 0.82 }],
      [
        { x: 0.4, y: 0.42 }, { x: 0.4, y: 0.78 }, { x: 0.6, y: 0.78 },
        { x: 0.6, y: 0.42 }, { x: 0.4, y: 0.42 },
      ],
      [{ x: 0.4, y: 0.78 }, { x: 0.4, y: 0.88 }],
      [{ x: 0.5, y: 0.78 }, { x: 0.5, y: 0.88 }],
      [{ x: 0.6, y: 0.78 }, { x: 0.6, y: 0.88 }],
      [
        { x: 0.72, y: 0.12 }, { x: 0.72, y: 0.72 }, { x: 0.9, y: 0.72 },
        { x: 0.9, y: 0.12 }, { x: 0.72, y: 0.12 },
      ],
      [{ x: 0.72, y: 0.72 }, { x: 0.72, y: 0.82 }],
      [{ x: 0.78, y: 0.72 }, { x: 0.78, y: 0.82 }],
      [{ x: 0.84, y: 0.72 }, { x: 0.84, y: 0.82 }],
      [{ x: 0.9, y: 0.72 }, { x: 0.9, y: 0.82 }],
      [
        { x: 0.44, y: 0.12 }, { x: 0.44, y: 0.32 }, { x: 0.56, y: 0.32 },
        { x: 0.56, y: 0.12 },
      ],
    ];
  }
  if (s === "monogram" || s.includes("monogram") || s.includes("letter") || s.includes("initial")) {
    return [
      [{ x: 0.22, y: 0.88 }, { x: 0.5, y: 0.48 }],
      [{ x: 0.78, y: 0.88 }, { x: 0.5, y: 0.48 }],
      [{ x: 0.5, y: 0.48 }, { x: 0.5, y: 0.12 }],
      [
        { x: 0.5, y: 0.95 }, { x: 0.92, y: 0.5 }, { x: 0.5, y: 0.05 },
        { x: 0.08, y: 0.5 }, { x: 0.5, y: 0.95 },
      ],
    ];
  }
  if (s === "butterfly" || s.includes("butterfly")) {
    return [
      [{ x: 0.5, y: 0.22 }, { x: 0.5, y: 0.78 }],
      [
        { x: 0.5, y: 0.55 }, { x: 0.28, y: 0.78 }, { x: 0.12, y: 0.62 },
        { x: 0.22, y: 0.42 }, { x: 0.5, y: 0.48 },
      ],
      [
        { x: 0.5, y: 0.42 }, { x: 0.28, y: 0.28 }, { x: 0.18, y: 0.18 },
        { x: 0.32, y: 0.22 }, { x: 0.5, y: 0.35 },
      ],
      [
        { x: 0.5, y: 0.55 }, { x: 0.72, y: 0.78 }, { x: 0.88, y: 0.62 },
        { x: 0.78, y: 0.42 }, { x: 0.5, y: 0.48 },
      ],
      [
        { x: 0.5, y: 0.42 }, { x: 0.72, y: 0.28 }, { x: 0.82, y: 0.18 },
        { x: 0.68, y: 0.22 }, { x: 0.5, y: 0.35 },
      ],
    ];
  }
  if (s === "cat" || s.includes("cat")) {
    return [
      [
        { x: 0.28, y: 0.28 }, { x: 0.28, y: 0.52 }, { x: 0.72, y: 0.52 },
        { x: 0.72, y: 0.28 }, { x: 0.28, y: 0.28 },
      ],
      circle(0.5, 0.68, 0.16, 12),
      [{ x: 0.38, y: 0.78 }, { x: 0.34, y: 0.92 }, { x: 0.46, y: 0.8 }],
      [{ x: 0.62, y: 0.78 }, { x: 0.66, y: 0.92 }, { x: 0.54, y: 0.8 }],
      [{ x: 0.34, y: 0.28 }, { x: 0.34, y: 0.1 }],
      [{ x: 0.44, y: 0.28 }, { x: 0.44, y: 0.1 }],
      [{ x: 0.56, y: 0.28 }, { x: 0.56, y: 0.1 }],
      [{ x: 0.66, y: 0.28 }, { x: 0.66, y: 0.1 }],
      [{ x: 0.72, y: 0.4 }, { x: 0.88, y: 0.55 }, { x: 0.92, y: 0.42 }],
    ];
  }
  return [
    [
