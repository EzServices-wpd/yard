...
/**
 * Deck sheet only when the primary stock is already sheet/board scale.
 * Single-stock craft queries (straws, sticks, toothpicks) stay on that stock —
 * the road is densified as primary members, never a foreign cardboard sheet.
...
export function isSingleStockCraft(item: CatalogItem): boolean {
  return (
    ...
    ((item.dims.diameter ?? 1) < 0.45 && item.formFactor !== "pipe")
  );
}
...
function withBridgeDeck(project: YardProject): YardProject {
  ...
  if (!sheet) {
    ...
    const ties = Math.max(5, Math.min(16, Math.round(span / Math.max(item.dims.length ?? 4.5, 2.5))));
    ...
    for (const z of [z0 + depth * 0.15, (z0 + z1) / 2, z1 - depth * 0.15]) {
      extras.push(
