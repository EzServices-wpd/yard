...
export function ExportDialog({ project, plan, onClose }: { project: YardProject; plan: BuildPlan; onClose: () => void }) {
  ...
  function exportPdf() {
    setPdfBusy(true); setDlNote(null);
    try {
    ...
    <div className="pointer-events-auto fixed inset-0 z-[60] flex flex-col bg-paper text-ink">
      <header className="flex shrink-0 items-center justify-between gap-3 border-b border-rule px-4 py-3 print:hidden">
        ...
        <div className="flex shrink-0 items-center gap-2">
          <button type="button" onClick={() => void copyMd()} className="h-10 rounded-md border border-rule px-3 text-sm text-ink">{copied ? "Copied" : "Copy"}</button>
          <button type="button" onClick={downloadMd} className="h-10 rounded-md border border-rule px-3 text-sm text-ink">.md</button>
          <button type="button" onClick={exportPdf} disabled={pdfBusy} className="h-10 rounded-md bg-ink px-3.5 text-sm font-medium text-paper disabled:opacity-50">{pdfBusy ? "Building…" : "PDF"}</button>
          <button type="button" onClick={onClose} className="grid size-10 place-items-center text-ink-muted hover:text-ink" aria-label="Close"><X className="size-4" /></button>
        </div>
      </header>
        ...
        <article className="mx-auto max-w-2xl px-5 py-8 sm:px-8">
          ...
          {pdfUrl && (
            <object data={pdfUrl} type="application/pdf" className="mt-6 h-[70vh] w-full border border-rule" title="Plan PDF">
              <p className="p-4 text-sm text-ink-muted">PDF built. <a href={pdfUrl} download={`${slugPlan(project.name)}-plan.pdf`} className="underline">Save the file</a></p>
            </object>
          )}
          {render?.url && (
          ...
          {plan.cutList.length > 0 && (
            <section className="mt-8">
              ...
              <table className="mt-3 w-full text-left text-sm">
                <thead className="text-xs uppercase tracking-wider text-ink-muted"><tr><th className="py-1 font-medium">Qty</th><th className="py-1 font-medium">Part</th><th className="py-1 font-medium">Size</th></tr></thead>
                <tbody>
                  {plan.cutList.map((c) => (
                    <tr key={c.id} className="border-t border-rule">
                      <td className="py-1.5 font-mono">{c.quantity}</td>
                      <td className="py-1.5">{c.name}</td>
                      <td className="py-1.5 font-mono text-ink-muted">{c.lengthIn}" × {c.widthIn}" × {c.thicknessIn}"</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {plan.bom.length > 0 && (
            <section className="mt-8">
              ...
              <p className="mt-1 text-xs text-ink-muted">{plan.totals.pieces} pieces · {usd(plan.totals.estCostUsd)} estimated</p>
              <ul className="mt-3 space-y-2 text-sm">
                {plan.bom.map((b, i) => <li key={i}>{b.quantity} {b.unit} · {b.name}{b.estimatedCost != null ? ` · ${usd(b.estimatedCost)}` : ""}</li>)}
              </ul>
            </section>
          )}
          <section className="mt-8">
            ...
            <ol className="mt-4 space-y-6">
              {plan.instructions.map((s) => (
                <li key={s.step} className="flex gap-4 border-t border-rule pt-4">
                  ...
                  <div>
                    <p className="font-medium"><span className="font-mono text-ink-muted">{String(s.step).padStart(2, "0")}</span> {s.title}</p>
